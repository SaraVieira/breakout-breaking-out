...
## `level_number` is 1-based, matching GameState.level. It wraps so a long run
## loops back to the first level instead of running dry.
func build(level_number: int) -> void:
	clear()
	if levels.is_empty():
		return

	current_index = posmod(level_number - 1, levels.size())
